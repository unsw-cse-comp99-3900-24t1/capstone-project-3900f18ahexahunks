...
const { getGfs, saveToMongo } = require("../../db");
...
const convertPdfToUbl = async (req, res) => {
  ...
  try {
    ...
    if (error.message.includes("Failed to connect to MongoDB")) {
      res
        .status(500)
        .json({ error: "Server error, including MongoDB connection errors" });
      ...
      error.message.includes(
        "Validation object with the same name already exists"
      )
    ) {
      res
        .status(401)
        .json({ error: "Validation object with the same name already exists" });
    } else {
...
const convertGuiToUbl = async (req, res) => {
  ...
  if (!guiData) {
    return res
      .status(400)
      .json({ error: "Bad request, no GUI data provided or missing fields" });
  }
  ...
  try {
    ...
    const guiId = await saveToMongo(
      JSON.stringify(guiData),
      guiFilename,
      guiMetadata,
      "application/json"
    );
